...
silver_edge_goalies_summary = spark.table("silver.fact_edge_stats_goalies_summary")
gold_players = spark.table("gold.dim_players")
gold_date = spark.table("gold.dim_date")


summary_with_date = silver_edge_goalies_summary.withColumn(
    "season_rep_date",
...
summary_with_player = (
    ...
    .select(
        ...
        "e.goals_against", "e.goals_against_league_avg", "e.goals_against_percentile",
        "e.save_pctg", "e.save_pctg_league_avg", "e.save_pctg_percentile",
        "e.saves", "e.saves_league_avg", "e.saves_percentile",
...
fact_edge_stats_goalies_summary = (
    ...
    .select(
        ...
        "e2.goals_against", "e2.goals_against_league_avg", "e2.goals_against_percentile",
        "e2.save_pctg", "e2.save_pctg_league_avg", "e2.save_pctg_percentile",
        "e2.saves", "e2.saves_league_avg", "e2.saves_percentile",
    )
)
...
fact_edge_stats_goalies_summary.write.format("delta").mode("overwrite").saveAsTable("gold.fact_edge_stats_goalies_summary")
print(f"Table gold.fact_edge_stats_goalies_summary saved with {fact_edge_stats_goalies_summary.count()} rows!")
...
# --- Verification ---
row_count_check = silver_edge_goalies_summary.count() == fact_edge_stats_goalies_summary.count()
null_rate_check = fact_edge_stats_goalies_summary.select(
